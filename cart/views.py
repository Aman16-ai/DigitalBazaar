...
def mycart(request):
    ...
    print(
        f"Cart Total : {userCart.getCartTotal} Total Items : {userCart.getCartTotalItems}")
    for item in allitem:
        print(
            f"Product : {item.product.title[:5]} , Price :{item.product.getFinalPrice} and Quantity : {item.quantity}")
